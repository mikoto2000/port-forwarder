...
GOARCH_LINUX_AMD := amd64
GOARCH_LINUX_ARM := arm64
GOARCH_WINDOWS := amd64
WINDOWS_BINARY_NAME := ${APP_NAME}-windows-${GOARCH_WINDOWS}.exe
LINUX_AMD_BINARY_NAME := ${APP_NAME}-linux-${GOARCH_LINUX_AMD}
LINUX_ARM_BINARY_NAME := ${APP_NAME}-linux-${GOARCH_LINUX_ARM}
...
build-all: build-windows build-linux-amd build-linux-arm build-darwin-arm build-darwin-amd
...
build-linux-amd: build/${LINUX_AMD_BINARY_NAME}
build/${LINUX_AMD_BINARY_NAME}: ${WATCH_SRC}
	GOOS=linux GOARCH=${GOARCH_LINUX_AMD} go build -ldflags=${LD_FLAGS} -trimpath -o build/${LINUX_AMD_BINARY_NAME}

build-linux-arm: build/${LINUX_ARM_BINARY_NAME}
build/${LINUX_ARM_BINARY_NAME}: ${WATCH_SRC}
	GOOS=linux GOARCH=${GOARCH_LINUX_ARM} go build -ldflags=${LD_FLAGS} -trimpath -o build/${LINUX_ARM_BINARY_NAME}
